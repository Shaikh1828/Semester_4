...
int64 BintoDec(const std::string& bin) 
{
    int64 value = 0;
...
int64 rotate_right(int64 x, int n) 
{
    ...
    int64 right_shifted = x >> n;
    int64 left_rotation_count = 64 - n;
    int64 left_shifted = x << left_rotation_count;
    int64 result = right_shifted | left_shifted;
...
int64 maj(int64 a, int64 b, int64 c) 
{
    ...
    int64 result = and_ab ^ and_bc ^ and_ca;
...
int64 Ch( int64 e, int64 f, int64 g ) 
{
    ...
    int64 and_not_e_g = not_e & g;

    int64 result = and_ef ^ and_not_e_g;
...
{
    // Rotate e right by 14 bits, 18 bits, and 41 bits
    int64 rotate_14 = rotate_right(e, 14);
    int64 rotate_18 = rotate_right(e, 18);
    int64 rotate_41 = rotate_right(e, 41);

    // Calculate the result using bitwise XOR (^) to combine the three rotation results
    int64 result = rotate_14 ^ rotate_18 ^ rotate_41;
...
{
    // Rotate a right by 28 bits, 34 bits, and 39 bits
    int64 rotate_28 = rotate_right(a, 28);
    int64 rotate_34 = rotate_right(a, 34);
    int64 rotate_39 = rotate_right(a, 39);

    // Calculate the result using bitwise XOR (^) to combine the three rotation results
    int64 result = rotate_28 ^ rotate_34 ^ rotate_39;
...
string SHA512(string myString)
{
	...
	string lengthbits = bitset<128>(orilen).to_string();

	str_pr = str_pr + bitset<128>(orilen).to_string();
	
	cout << "Total:" << s1024.length() << " \nTo be added : "<< tobeadded << endl ;
...
string generateSHA512Hash(const string& inputFilePath, const string& outputFilePath) 
{
    ...
    string hashResult = SHA512(fileContent);

    ofstream outputFile(outputFilePath);
    if (!outputFile.is_open()) 
	{
        cerr << "Error opening " << outputFilePath << " for writing." << endl;
        return "";
    }

    outputFile << hashResult;
    outputFile.close();

    //cout << "Hash saved in " << outputFilePath << endl;
